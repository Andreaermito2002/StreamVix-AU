...
export class KitsuProvider {
  ...
  parseKitsuId(kitsuIdString: string): { kitsuId: string; seasonNumber: number, episodeNumber: number | null; isMovie: boolean } {
    ...
    // Movie format: kitsu:ID
    if (parts.length === 2) {
      return { kitsuId, seasonNumber: 1, episodeNumber: 1, isMovie: true };
    } 
    // Series format without season: kitsu:ID:EPISODE
    else if (parts.length === 3) {
      return { kitsuId, seasonNumber: 1, episodeNumber: parseInt(parts[2]), isMovie: false };
    }
    // Full series format: kitsu:ID:SEASON:EPISODE
